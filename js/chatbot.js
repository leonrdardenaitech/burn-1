...
document.addEventListener('DOMContentLoaded', () => {
    ...
    container.innerHTML = `
        <button id="burn1-chatbot-trigger" onclick="toggleChatbot()">
        ...
        <div id="burn1-chatbot-modal" class="hidden">
            ...
            <div class="bg-black p-3 border-b-2 border-cyan-400 flex justify-between items-center text-xs font-bold font-mono">
                ...
                <button onclick="toggleChatbot()" class="text-cyan-400 font-black hover:text-red-400">✕</button>
            </div>
            ...
            <div class="p-2 bg-black border-t border-cyan-500/30 flex flex-wrap gap-1">
                <button class="chip-btn" onclick="sendChip('What is $50 Flat?')">$50 Pricing</button>
                <button class="chip-btn" onclick="sendChip('How do Kiosks work?')">Kiosks</button>
                <button class="chip-btn" onclick="sendChip('Local AI Setup')">Local AI</button>
                <button class="chip-btn" onclick="sendChip('Go to Onboarding')">Onboarding</button>
            </div>

            <!-- INPUT FIELD -->
            <div class="p-2 bg-black border-t-2 border-cyan-400 flex gap-2">
                <input type="text" id="bot-input" placeholder="Type query..." onkeypress="handleKey(event)" class="w-full bg-black border border-cyan-400 px-3 py-2 text-xs text-white focus:outline-none focus:border-emerald-400">
                <button onclick="sendUserMsg()" class="bg-cyan-400 text-black font-bold px-3 text-xs uppercase hover:bg-emerald-400">SEND</button>
            </div>
        </div>
    `;
...
function sendUserMsg() {
    ...
    setTimeout(() => {
        ...
        if (query.includes('50') || query.includes('price') || query.includes('cost')) {
            ...
            bDiv.innerHTML = `<strong>[SYSTEM]:</strong> Our Privacy-First Local AI Vault configures air-gapped LLMs on your internal hardware with zero cloud leaks.`;
        } else if (query.includes('onboard') || query.includes('receipt') || query.includes('key')) {
            bDiv.innerHTML = `<strong>[SYSTEM]:</strong> You can activate receipt keys and download assets directly at <a href="onboarding.html" class="underline text-emerald-400">burn-1.com/onboarding.html</a>.`;
        } else {
            bDiv.innerHTML = `<strong>[SYSTEM]:</strong> Query received. All assets are flat $50 standalone zip downloads. Visit our <a href="onboarding.html" class="underline text-emerald-400">Onboarding Portal</a> for instant dispatch.`;
        }
